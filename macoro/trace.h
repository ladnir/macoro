...
namespace macoro
{
	struct traceable
	{
		...
		// point to the first (active) child.
		// The child will set this value.
		//traceable* m_child = 0;
		...
		//{
		//	std::mutex m_mutex;
		//	std::vector<traceable*> m_children;
		//};

		//std::optional<multi_child> m_multi_child;

		//void set_parent(traceable* parent, std::source_location l)
		//{
		//	assert(mParent == nullptr);
		//	if (parent)
		//	{
		//		parent->mFile = l.file_name();
		//		parent->mLine = l.line();

		//		mParent = parent;
		//		assert(parent->mChild == nullptr);
		//		parent->mChild = this;
		//	}
		//}

		//void set_detatched(traceable* parent, std::source_location l)
		//{
		//	assert(mParent == nullptr);
		//	if (parent)
		//	{
		...
		//		mParent = parent;
		//		if (mParent->m_multi_child.has_value() == false)
		//			mParent->m_multi_child.emplace();

		//		std::lock_guard<std::mutex> lck(mParent->m_multi_child->m_mutex);
		//		mParent->m_multi_child->m_children.push_back(this);
		//	}
		//}

		//~traceable()
		//{
		//	// we are a suspend child of some coro
		//	if (mParent)
		//	{
		//		if (mParent->mChild == this)
		//		{
		//			// we are a normal child.
		//			mParent->mChild = nullptr;
		//		}
		//		else
		//		{
		//			auto& children = mParent->m_multi_child;
		//			assert(children.has_value());
		//			std::lock_guard<std::mutex> lck(children->m_mutex);

		//			auto iter = std::find(
		//				children->m_children.begin(),
		//				children->m_children.end(),
		//				this);
		//			assert(iter != children->m_children.end());
		//			std::swap(*iter, children->m_children.back());
		//			children->m_children.pop_back();
		//		}
		//	}
