...
namespace macoro
{
    ...
    public:
        ...
        // Set the default for the provided key. Keys do not include the leading `-`.
        void setDefault(std::string key, std::int64_t value) { setDefault(key, std::to_string(value)); }

        // Set the default for the provided key. Keys do not include the leading `-`.
        void setDefault(std::vector<std::string> keys, std::int64_t value) { setDefault(keys, std::to_string(value)); }
        ...
        CommandLineParserError error(std::vector<std::string> names) const
        {
            if (names.size() == 0)
            ...
            else
            {
                std::stringstream ss;
                ss << "{ " << names[0];
                for (std::size_t i = 1; i < static_cast<std::size_t>(names.size()); ++i)
                    ss << ", " << names[i];
                ss << " }";
